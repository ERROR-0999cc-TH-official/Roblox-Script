-- ===================================================
-- Script Loading...
warn("[ ❖ ] Script Loading...") -- สีเหลือง

warn("[ ❖ ] Script Loading Service")
...
local transparencyLevel = 0.7 -- ระดับความโปร่งใส (0 = ทึบ, 1 = หายหมด)
...
local function toggleXray(on)
	...
	if on then
		-- ทำให้ทุกออปเจ็กต์ที่มีอยู่โปร่งใสและสร้างขอบ
...
-- ตรวจจับออปเจ็กต์ที่ถูกสร้างใหม่ใน Workspace
...
-- ตรวจสอบสถานะ toggle ทุก 0.2 วินาที
task.spawn(function()
...
-- ===================================================  
-- Page: Shader
local ShaderPage = Instance.new("Frame")
ShaderPage.Size = UDim2.new(1, 0, 1, 0)
ShaderPage.BackgroundTransparency = 1
ShaderPage.Visible = false
ShaderPage.Active = true
ShaderPage.Parent = contentFrame
pages["• Shader"] = ShaderPage
...
local function showDisconnect(reason)  
    pcall(function()  
        MES:Kick(reason or "ER GUI : ยกเลิกการเชื่อมต่อแล้ว") -- ข้อความเริ่มต้นถ้าไม่มี showDisconnect  
    end)  
end  
  
showDisconnect("ER GUI : ยกเลิกการเชื่อมต่อแล้ว")  -- ข้อความที่แสดง

end)

-- ===================================================  
local getScript2 = createToggle(wsjpPage, "[ ◆ ] Simple Shader", cheatStartY + 40, function()
...
-- ===================================================  
local getScript3 = createToggle(wsjpPage, "[ ◆ ] Pshade Ultimate Shader", cheatStartY + 80, function()
...
-- ===================================================  
local getScript7 = createToggle(wsjpPage, "Script 7", cheatStartY + 240, function()
...
-- ===================================================  
local getScript8 = createToggle(wsjpPage, "Script 8", cheatStartY + 280, function()
...
containerMG.Position = UDim2.new(0, 20, 0, 155)
...
local UIPadding = Instance.new("UIPadding", playerListFrame)
...
-- Loop ตรวจสอบชื่อเพลงทุก 0.5 วินาที
task.spawn(function()
    while task.wait(0.5) do
...
-- ===================================================  
-- Sidebar Buttons
local menuItems = {"• Player", "• Visual", "• Shader", "• Teleport", "• Music"}
